...
namespace 调试者
{
    public partial class TS调试者 : Form
    {
        ...
        private void fs发送(string _fs)
        {
            if (_fs.Length > 0)
            {
                try
                {
                    ...
                    {
                        _fs=_fs.Insert(0, textBox13.Text.ToString());           
                    }

                    if (checkBox2.Checked==true&&CheckSum.Checked==false)
                    {                  
                          _fs = _fs.Insert(_fs.Length, textBox14.Text.ToString());
                    ...
                    if (CheckSum.Checked == true && checkfs16进制发送.Checked==true)
                    {
                        ...
                        Array.Copy(bt, 0, bt2, 0, bt.Length);
        ...
        protected override void OnClosed(EventArgs e)
        {
            ...
            elementM.SetAttribute("Text", textBox13.Text);
            elementH.AppendChild(elementM);
            elementM = doc.CreateElement("帧尾");
            elementM.SetAttribute("Text", textBox14.Text);
            elementH.AppendChild(elementM);
